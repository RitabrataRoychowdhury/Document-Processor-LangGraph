...
class ComprehensiveHealthChecker:
    ...
    def get_overall_health(self) -> SystemHealth:
        """Get overall system health by running all health checks."""
        try:
            components = {}
            ...
            for check_name, check_func in self.health_checks.items():
                try:
                    components[check_name] = check_func()
                except Exception as e:
            ...
            critical_count = sum(1 for comp in components.values() if comp.is_critical)
            warning_count = sum(1 for comp in components.values() if comp.status == HealthStatus.WARNING)
            ...
            summary = {
                ...
                'healthy_components': len([c for c in components.values() if c.is_healthy]),
